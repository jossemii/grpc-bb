...
class TestBlockBuilder(unittest.TestCase):
    ...
    def test_typical_complex_object_with_duplicate_blocks(self):
        ...
        c = Test()
        c.t1 = b''.join([b'ct1' for i in range(100)])
        c.t2 = block1.SerializeToString()

        _object = Test()
        _object.t1 = b''.join([b'mc1' for i in range(100)])
        _object.t2 = block2.SerializeToString()
        _object.t4.append(b)
        _object.t4.append(c)
        _object.t5 = b'final'
        ...
        print(f"Block builder ok. test complex obj with duplicate blocks.")
        # Read the buffer.
        buffer = b''
        with open(os.path.join(cache_dir, '_.json'), 'r') as f:
            _json = json.load(f)
